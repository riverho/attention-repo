...
if (mcpConfigOutput.includes("ATTENTION_REPO_PATH")) {
  throw new Error(`Default mcp-config should be adaptive, got: ${mcpConfigOutput}`);
}

const pinnedMcpConfigOutput = run(["mcp-config", "codex", "--repo", "/tmp/test-repo"]);
if (!pinnedMcpConfigOutput.includes('ATTENTION_REPO_PATH = "/tmp/test-repo"')) {
  throw new Error(`Expected explicit repo path in mcp-config output: ${pinnedMcpConfigOutput}`);
}
