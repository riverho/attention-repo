...
def _generate_entity_pipeline(repo: Path, entity_id: str, file_path: str) -> str:
    """Generate and write a per-entity GitHub Actions workflow file."""
    entity_lower = entity_id.lower()
    workflow_dir = repo / ".github" / "workflows" / "entity"
    workflow_dir.mkdir(parents=True, exist_ok=True)
    workflow_path = workflow_dir / f"{entity_id}.yml"

    workflow_content = f'''name: "{entity_id} pipeline"
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Verify entity file exists
        run: |
          test -f "{file_path}" || {{
            echo "ERROR: {entity_id} file {file_path} not found"
            exit 1
          }}
      ...
      - name: Run Node tests
        run: |
          npm install --silent
          node tests/test_package_contract.js
          node tests/test_bun_cli.js
          node tests/test_mcp_integration.js
'''
    workflow_path.write_text(workflow_content, encoding="utf-8")
    return str(workflow_path.relative_to(repo))
...
def cmd_register_new_entity(args: argparse.Namespace) -> None:
    ...
    # Auto-generate entity pipeline if --ci-cd not provided
    ci_cd = args.ci_cd
    if not ci_cd:
    ...
    if not pipeline_path.exists():
        raise ValueError(f"ci_cd pipeline does not exist: {ci_cd}")
    ...
    print(f"Registered new staged entity in {map_path}: {args.id} [status=staged]")
...
def cmd_finalize_change(args: argparse.Namespace) -> None:
    ...
    full_head = run_git(repo, "rev-parse", "HEAD")

    # Lifecycle v2: promote staged entities to active, stamp canonical info
    staged_promotions = []
    affected_entities = declaration.get("affected_entities", [])

    if affected_entities:
        map_path = repo / "!MAP.md"
        registry = extract_entity_registry(read_text(map_path))
        entities_by_id = {e["id"]: e for e in registry["entities"]}

        for eid in affected_entities:
            entity = entities_by_id.get(eid)
            if entity and entity.get("status") == "staged":
                entity["status"] = "active"
                entity["canonical_commit"] = short_head
                entity["canonical_sha"] = full_head
                entity["route_version"] = "1.0.0"
                staged_promotions.append(eid)

        if staged_promotions:
            write_entity_registry(map_path, registry)
            print(f"Promoted staged → active: {', '.join(staged_promotions)}")
    ...
    report += (
        ...
        f"- Notes: {args.notes}\n\n"
        "## Lifecycle v2\n"
        f"- Staged → Active: {', '.join(staged_promotions) or 'none'}\n"
        f"- Canonical Commit: {short_head}\n"
        f"- Canonical SHA: {full_head}\n\n"
        "## Git\n"
        f"- HEAD: {short_head}\n"
        f"- Working tree:\n{changed or '<clean>'}\n"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_reg.add_argument("--ci-cd", default="", help="Pipeline path; if omitted, auto-generated as .github/workflows/entity/{id}.yml")
